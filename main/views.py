...
from .serializers import (
    FlatPagesSerializer, TeacherStudentChatSerializer, ChairmanDashboardSerializer, ChairmanSerializer, 
    TeacherSerializer, CourseExamSerializer, AttemptExamSerializer, QuestionSerializer, ExamSerializer, 
    StudentDashboardSerializer, NotificationSerializer, TeacherDashboardSerializer, CategorySerializer, CourseSerializer, 
    StudentCourseEnrollSerializer, StudentSerializer, StudentAssignmentSerializer, AddExamSerializer)
from . import models
...
def verify_teacher_via_otp(request, teacher_id):
    ...
    verify = models.Teacher.objects.filter(
...
def verify_student_via_otp(request, student_id):
    otp_digit = request.POST.get('otp_digit')
    verify = models.Student.objects.filter(
        id=student_id, otp_digit=otp_digit).first()
    if verify:
        models.Student.objects.filter(
            id=student_id, otp_digit=otp_digit).update(verify_status=True)
...
class MyAssignmentList(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        models.Notification.objects.filter(
            student=student, notif_for='student', notif_subject='assignment').update(notif_read_status=True)
        return models.StudentAssignment.objects.filter(student=student)


class UpdateAssignments(generics.RetrieveUpdateDestroyAPIView):
    queryset = models.StudentAssignment.objects.all()
    serializer_class = StudentAssignmentSerializer
...
# add exam
class AddExam(generics.ListCreateAPIView):
    queryset = models.Exam.objects.all()
...
class TeacherExamDetail(generics.RetrieveUpdateDestroyAPIView):
    queryset = models.Exam.objects.all()
    serializer_class = ExamSerializer
...
class ExamQuestionList(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        ...
        if 'limit' in self.kwargs:
            return models.ExamQuestions.objects.filter(exam=exam).order_by('id')[:1]
        elif 'question_id' in self.kwargs:
            current_question = self.kwargs['question_id']
            return models.ExamQuestions.objects.filter(exam=exam, id__gt=current_question).order_by('id')[:1]

        else:
            return models.ExamQuestions.objects.filter(exam=exam)


class CourseExamList(generics.ListCreateAPIView):
    queryset = models.CourseExam.objects.all()
    ...
    def get_queryset(self):
        if 'course_id' in self.kwargs:
            course_id = self.kwargs['course_id']
            course = models.Course.objects.get(pk=course_id)
            return models.CourseExam.objects.filter(course=course)


def fetch_exam_assign_status(request, exam_id, course_id):
    exam = models.Exam.objects.filter(id=exam_id).first()
    course = models.Course.objects.filter(id=course_id).first()
    assignStatus = models.CourseExam.objects.filter(
        course=course, exam=exam).count()
    if assignStatus:
        return JsonResponse({'bool': True})
    else:
        return JsonResponse({'bool': False})
...
def fetch_exam_attempt_status(request, exam_id, student_id):
    exam = models.Exam.objects.filter(id=exam_id).first()
    student = models.Student.objects.filter(id=student_id).first()
    attemptStatus = models.AttemptExam.objects.filter(
        student=student, question__exam=exam).count()
    print(models.AttemptExam.objects.filter(
...
def fetch_exam_result(request, exam_id, student_id):
    exam = models.Exam.objects.filter(id=exam_id).first()
    student = models.Student.objects.filter(id=student_id).first()
    total_questions = models.ExamQuestions.objects.filter(exam=exam).count()
    total_attempted_questions = models.AttemptExam.objects.filter(
        exam=exam, student=student).values('student').count()
    attempted_questions = models.AttemptExam.objects.filter(
...
class AttemptExamList(generics.ListCreateAPIView):
    queryset = models.AttemptExam.objects.all()
    ...
    def get_queryset(self):
        if 'exam_id' in self.kwargs:
            exam_id = self.kwargs['exam_id']
            exam = models.Exam.objects.get(pk=exam_id)
            return models.AttemptExam.objects.raw(f'SELECT * FROM main_attemptexam WHERE exam_id={int(exam_id)} GROUP by student_id')
...
class MyTeacherList(generics.ListAPIView):
    ...
    def get_queryset(self):
        if 'student_id' in self.kwargs:
            student_id = self.kwargs['student_id']
            sql = f"SELECT * FORM main_course as c,main_studentcourseenrollment as e,main_teacher as t WHERE c.teacher_id=t.id AND e.course_id=c.id AND e.student_id={student_id} GROUP BY c.teacher_id"
